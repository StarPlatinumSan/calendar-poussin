const TIMING_OPTIONS = [
	...
	{ id: '0m', label: 'Au moment du debut', offsetMs: 0 },
]
...
export function startWebPushReminderScheduler({
	webPush,
		...
		try {
			...
			for (const preference of enabledPreferences) {
				...
				for (const timingId of preference.timings) {
					...
					const shouldSend = await reminderDeliveryLogRepository.registerDelivery(
						preference.userId,
						event.id,
						timingId,
						triggerMs,
					...
					await Promise.all(
						subscriptions.map(async (subscription) => {
							try {
								...
								console.error('[web-push] send failed', JSON.stringify({
									statusCode,
